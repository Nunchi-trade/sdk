use super::{
    Application, Block, Finalization, Mempool, Scheme, SharedState, EPOCH, EPOCH_LENGTH, NAMESPACE,
};
...
const PAGE_CACHE_PAGE_SIZE: NonZero<u16> = NZU16!(4_096);
    ...
    S: Strategy,
{
    pub async fn new(context: E, cfg: Config<B, P, S>) -> Self {
        ...
        let app = Application::new(cfg.mempool, cfg.state);
    ...
    async fn run(
        ...
        if let Err(e) = try_join_all(vec![buffer_handle, marshal_handle, consensus_handle]).await {
            error!(?e, "coinschain engine failed");
        } else {
            warn!("coinschain engine stopped");
        }
    }
}
